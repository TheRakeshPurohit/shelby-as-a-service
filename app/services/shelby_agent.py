            ...
            match workflow:
                case 1:
                    ...
                    if topic == 0:
                        ...
                        for key, value in self.vectorstore_namespaces.items():
...
#         if isinstance(self.vectorstore_namespaces, dict):
#             content_strs = [f"{index + 1}. {key}: {value}" for index, (key, value) in enumerate(self.vectorstore_namespaces.items())]
...
#         logit_bias_weight = 100
#         logit_bias = {str(k): logit_bias_weight for k in range(15, 15 + len(self.vectorstore_namespaces) + 1)}
...
#             if token_count > self.max_docs_tokens:
...
#         while embeddings_tokens > self.max_docs_tokens:
...
#         while len(sorted_documents) > self.max_docs_used:
#             if soft_count > 1:
#                 for idx, document in reversed(list(enumerate(sorted_documents))):
#                     if document['doc_type'] == 'soft':
#                         sorted_documents.pop(idx)
