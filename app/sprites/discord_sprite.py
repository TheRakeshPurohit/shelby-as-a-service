...
class DiscordSprite(BaseClass):
    ...
    manual_requests_enabled: bool = True  # Manual allows for @discord-sprite engagement
    auto_response_enabled: bool = False  # Auto responds automatically to relevant questions
    auto_response_cooldown: int = 10 # Time in minutes to cooldown between auto-responding to specific users
    auto_respond_in_threads: bool = False # Auto respond to messages from users in their conversations with sprite
    # Channel selection - at least one is required
    # Respond in all channels
    all_channels_enabled: bool = False
    all_channels_excluded_channels: Optional[list[int]] = None # Exclude channels to reply in
    # Respond in specific channels
    specific_channels_enabled: bool = True
    specific_channel_ids: Optional[list[int]] = None # Requires at least one channel if enabled
    user_daily_token_limit: int = 30000
    # Content
    welcome_message: str = 'ima tell you about the {}.'
    short_message: str = '<@{}>, brevity is the soul of wit, but not of good queries. Please provide more details in your request.'
    message_start: str = 'Running request... relax, chill, and vibe a minute.'
    message_end: str = 'Generated by: gpt-4. Memory not enabled. Has no knowledge of past or current queries. For code see https://github.com/ShelbyJenkins/shelby-as-a-service.'
    _PLATFROM: str = 'discord'
    #endregion
    
    def __init__(self, moniker, **kwargs):
        super().__init__(**kwargs)  
        self.modifier = f'{moniker}_{self._PLATFROM}'
        self.LoadVarsFromEnv(self.modifier)
        self.check_discord_config()
                
        self.shelby_agent = ShelbyAgent(self)  
        # self.log_service = LogService(f'{moniker}_discord_sprite', f'{moniker}_discord_sprite.log', level='INFO')

        self.run_discord_sprite()
    ...
    def run_discord_sprite(self):
        ...
        async def on_guild_join(guild):
            ...
            if channel:
                await channel.send(format_message(self.welcome_message, get_random_animal()))
        ...
        async def on_message(message):
            ...
            if self.bot.user.mentioned_in(message):
                ...
                if len(request.split()) < 4:
                    await message.channel.send(format_message(self.short_message, message.author.id))
                    return
                
                # Create thread
                thread = await message.create_thread(name=f"{get_random_animal()} {message.author.name}'s request", auto_archive_duration=60)

                await thread.send(self.message_start)
                ...
                if isinstance(request_response, dict) and 'answer_text' in request_response:
                    ...
                    await thread.send(self.message_end)
        ...
        async def check_server_for_channel_id(guild):
            ...
            for config_channel_id in self.specific_channel_ids:
    ...
    def check_discord_config(self):
        ...
        if hasattr(self, 'all_channels_excluded_channels') and isinstance(self.all_channels_excluded_channels, str):
            self.all_channels_excluded_channels = [int(id) for id in self.all_channels_excluded_channels.split(',') if id]
        if hasattr(self, 'specific_channel_ids') and isinstance(self.specific_channel_ids, str):
            self.specific_channel_ids = [int(id) for id in self.specific_channel_ids.split(',') if id]
        # "" are required for formating in github actions workflow, but they need to be removed for use by discord sprite
        self.welcome_message = self.welcome_message.strip('"')
        self.short_message = self.short_message.strip('"')
        self.message_start = self.message_start.strip('"')
        self.message_end = self.message_end.strip('"')
        
        # Calls CheckRequiredVars but excludes variables that aren't required
        required_vars = []  
        for var in vars(self.__class__):
            if var == 'all_channels_excluded_channels' and self.all_channels_enabled == False:
                continue
            if var == 'specific_channel_ids' and self.specific_channels_enabled == False:
                continue
        ...
        if not self.manual_requests_enabled and not self.auto_response_enabled:
            raise ValueError('Error: manual_requests_enabled and auto_response_enabled cannot both be False.')
