...
class DiscordSprite(BaseClass):
    ...
    discord_manual_requests_enabled: bool = True  # Manual allows for @discord-sprite engagement
    discord_auto_response_enabled: bool = False  # Auto responds automatically to relevant questions
    discord_auto_response_cooldown: int = 10 # Time in minutes to cooldown between auto-responding to specific users
    discord_auto_respond_in_threads: bool = False # Auto respond to messages from users in their conversations with sprite
    # Channel selection - at least one is required
    # Respond in all channels
    discord_all_channels_enabled: bool = False
    discord_all_channels_excluded_channels: Optional[list[int]] = None # Exclude channels to reply in
    # Respond in specific channels
    discord_specific_channels_enabled: bool = True
    discord_specific_channel_ids: Optional[list[int]] = None # Requires at least one channel if enabled
    ...
    _DISCORD_PLATFROM: str = 'discord'
    _SECRET_VARIABLES: list = ['discord_bot_token']
    _REQUIRED_VARIABLES: list = ['discord_bot_token']
    #endregion
    
    def __init__(self, moniker, **kwargs):
        super().__init__(**kwargs)  
        self.modifier = f'{self.deployment_name}_{moniker}_{self._DISCORD_PLATFROM}'
    ...
    def check_discord_config(self):
        ...
        if hasattr(self, 'discord_all_channels_excluded_channels') and isinstance(self.discord_all_channels_excluded_channels, str):
            self.discord_all_channels_excluded_channels = [int(id) for id in self.discord_all_channels_excluded_channels.split(',') if id]
        if hasattr(self, 'specific_channel_ids') and isinstance(discord_specific_channel_ids, str):
            discord_specific_channel_ids = [int(id) for id in discord_specific_channel_ids.split(',') if id]
        # "" are required for formating in github actions workflow, but they need to be removed for use by discord sprite
        discord_welcome_message = discord_welcome_message.strip('"')
        discord_short_message = discord_short_message.strip('"')
        discord_message_start = discord_message_start.strip('"')
        discord_message_end = discord_message_end.strip('"')
        
        # Calls CheckRequiredVars but excludes variables that aren't required
        required_vars = []  
        for var in vars(__class__):
            if var == 'all_channels_excluded_channels' and self.discord_all_channels_enabled == False:
                continue
            if var == 'specific_channel_ids' and self.discord_specific_channels_enabled == False:
                continue
            required_vars.append(var) 

        self.CheckRequiredVars(required_vars)

        if not self.discord_manual_requests_enabled and not self.discord_auto_response_enabled:
