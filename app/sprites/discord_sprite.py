...
class DiscordSprite(BaseClass):
    ...
    def __init__(self, moniker=None, **kwargs):
        super().__init__(**kwargs)
        self.moniker = moniker
        self.class_name = self.__class__.__name__
        
        self.LoadVarsFromEnv(moniker=self.moniker, class_name=self.class_name)
    ...
    def run_discord_sprite(self):
        ...
        async def on_guild_join(guild):
            ...
            if channel:
                await channel.send(format_message(self.discord_welcome_message, get_random_animal()))
        ...
        async def on_message(message):
            ...
            if self.bot.user.mentioned_in(message):
                ...
                if len(request.split()) < 4:
                    await message.channel.send(format_message(self.discord_short_message, message.author.id))
                    return
    ...
    def check_discord_config(self):
        ...
        if self.discord_all_channels_excluded_channels and isinstance(self.discord_all_channels_excluded_channels, str):
            self.discord_all_channels_excluded_channels = [int(id) for id in self.discord_all_channels_excluded_channels.split(',') if id]
        if self.discord_specific_channel_ids and isinstance(self.discord_specific_channel_ids, str):
            self.discord_specific_channel_ids = [int(id) for id in self.discord_specific_channel_ids.split(',') if id]
