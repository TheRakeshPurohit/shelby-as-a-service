...
class YourConfig():
    ...
    ### YourConfig is where you set your vars for local and container deployment ###
    ...
    def __init__(self):
        ...
        self.secret_names = ['STACKPATH_CLIENT_ID', 'STACKPATH_API_CLIENT_SECRET', 'OPENAI_API_KEY', 'PINECONE_API_KEY', 'DOCKER_TOKEN']
    ...
    def config_check(self, generic_config, deployment_target: str, log_agent: LoggerAgent):
        
        ### Configuration Checks ###
        
        # Specific checks for DeploymentEnvs
        if type(generic_config).__name__ == 'DeploymentEnvs':
            match deployment_target:
                case 'discord':
                    
                    if not generic_config.discord_manual_requests_enabled and not generic_config.discord_auto_response_enabled:
                        generic_config.log_agent.print_log_error('An error occurred in config_checks: if not self.discord_manual_requests_enabled and not self.discord_auto_response_enabled:')
                    
                    if generic_config.discord_auto_response_enabled and generic_config.discord_auto_response_cooldown == None:
                        generic_config.log_agent.print_log_error('An error occurred in config_checks: if self.discord_auto_response_enabled and self.discord_auto_response_cooldown == None:')
                    
                    if generic_config.discord_all_channels_enabled == generic_config.discord_specific_channels_enabled:
                        generic_config.log_agent.print_log_error('An error occurred in config_checks: if self.discord_all_channels_enabled == self.discord_specific_channels_enabled:')
                        
                    if generic_config.discord_specific_channels_enabled and not generic_config.discord_specific_channel_ids:
                        generic_config.log_agent.print_log_error('An error occurred in config_checks: if self.discord_specific_channels_enabled and not self.discord_specific_channel_ids:')

        # Checks all variables to make sure they're not None
        for var, value in vars(generic_config).items():
            if not var.startswith('_'):
                if value is None:
                    log_agent.print_log_error(f"{var} variable cannot be None")
...
class DeploymentConfig():
    ...
    class DeploymentEnvs:
        ...
        def __init__(self, your_config: YourConfig, deployment_target: str, log_agent: LoggerAgent):
